...
export default class ProductManager {
  ...
  post = async (productBody) => {
    try {
      ...
      let { title, description, price, thumnail, code, stock, category } =
        productBody;
  ...
  getProduct = async (pid) => {
    try {
      ...
      console.log(
        "Cannot get product by id in manager with mongoose: " + error
      );
    }
  };
  ...
  put = async (idProduct, product) => {
    try {
      ...
      console.log(
        "Cannot update product by id in manager with mongoose: " + error
      );
    }
  };
  ...
  delete = async (aId) => {
    try {
      ...
      console.log(
        "Cannot delete the product in manager with mongoose: " + error
      );
    }
  };
}
